...
class _ChatbotScreenState extends State<ChatbotScreen> {
  ...
  Future<void> _send(String text) async {
    ...
    // Try local handling first (instant, no network)
    ...
    try {
      ...
      final isOffline = e.toString().toLowerCase().contains('connect') ||
      ...
      appState.addBotText(
        isOffline
            ? "I'm offline right now. Here's what I can help with locally:\n"
              "• Check your queue status\n"
              "• Navigate to booking screen\n"
              "• Find clinics on the map"
            : "I had trouble getting a response. Please try again.",
        quickReplies: ['Check my queue status', 'Book an appointment'],
      );
    } finally {
      if (mounted) setState(() => _sending = false);
      _scrollToBottom();
