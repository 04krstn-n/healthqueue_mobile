...
class _ScoredClinic {
  ...
  const _ScoredClinic({required this.clinic, required this.distKm,
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  void initState() {
    ...
    // Real-time: refresh queue status and notifications every 3s
  ...
  Future<void> _loadClinics() async {
    try {
      ...
      if (mounted) setState(() { _clinics = list; _clinicsLoad = false; });
      _rebuildMarkers();
    } catch (_) {
      if (mounted) setState(() => _clinicsLoad = false);
    }
  }
  ...
  Future<void> _markAllRead() async {
    ...
    setState(() {
      _notifications = _notifications.map((n) => {...n as Map<String, dynamic>, 'isRead': true}).toList();
      _unreadCount   = 0;
    });
  }
  ...
  void _computeRecommendations() {
    ...
    final scored = _clinics.where((c) => c.status == 'open').map((c) {
      ...
      exp += wait == 0 ? ' · No wait'
          ...
          : ' · ~${wait.toInt()} min wait';
      return _ScoredClinic(clinic: c, distKm: dist, score: score, explanation: exp);
    }).toList()
                  ...
                  Padding(
                    ...
                    child: Row(children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => setState(() => _notifOpen = false),
                      ),
                    ]),
                  ),
  ...
  IconData _notifIcon(String? type) {
    switch (type) {
      case 'queue_joined':         return Icons.queue;
      case 'queue_called':         return Icons.notifications_active;
      case 'queue_completed':      return Icons.check_circle;
      case 'appointment_booked':   return Icons.calendar_today;
  ...
  Widget build(BuildContext context) {
    ...
    // Use the ActiveQueueStatus wrapper — fixes the type mismatch
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // Active queue card — uses qStatus.inQueue
                if (qStatus.inQueue) ...[
                  _sectionTitle('Your Active Queue'),
                  _ActiveQueueCard(queueEntry: qStatus.entry!),
                ...
                if (nextAppt != null) ...[
                  ...
                  _AppointmentReminderCard(appt: nextAppt),
                  const SizedBox(height: 20),
                ],
                ...
                _QuickActions(
                  onMap:        () => Navigator.pushNamed(context, AppRoutes.clinicMap),
                  onAiInsights: () => Navigator.pushNamed(context, AppRoutes.aiInsights),
                ),
                const SizedBox(height: 24),

                // Smart recommendations
                _sectionTitle('Recommended Clinics'),
                if (_clinicsLoad)
                  const Center(child: Padding(padding: EdgeInsets.all(24),
                      child: CircularProgressIndicator()))
                else if (_recommended.isEmpty)
                  _emptyCard('No open clinics found nearby')
                else
                  Column(
                    children: _recommended.map((r) => _RecommendationCard(
                      rec: r,
                    )).toList(),
                  ),
                ...
                // Mini map
                _sectionTitle('Nearby Clinics Map'),
                ClipRRect(
...
// ── Active Queue Card — receives QueueEntry directly ──────────────────────────
class _ActiveQueueCard extends StatelessWidget {
  final QueueEntry queueEntry;
  const _ActiveQueueCard({required this.queueEntry});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Icon(isCalled ? Icons.notifications_active : Icons.queue,
        ...
        if (isCalled) ...[
          ...
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 8),
              Expanded(child: Text('Please proceed to the counter now!',
                  style: TextStyle(
                      color: Colors.white,
                      fontSize: 12,
...
class _AppointmentReminderCard extends StatelessWidget {
  final dynamic appt;
  const _AppointmentReminderCard({required this.appt});
  ...
  Widget build(BuildContext context) {
    ...
    final svcName = (appt.serviceName?.isNotEmpty == true)
        ...
        : (appt.department?.isNotEmpty == true ? appt.department as String : 'Appointment');
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('$dateStr${slot.isNotEmpty ? ' · $slot' : ''}',
              style: const TextStyle(
                  fontSize: 11,
...
class _QuickActions extends StatelessWidget {
  final VoidCallback onMap, onAiInsights;
  const _QuickActions({required this.onMap, required this.onAiInsights});

  @override
  Widget build(BuildContext context) {
    return Row(children: [
      Expanded(child: _QABtn(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(
            ...
            Text(rec.explanation, style: const TextStyle(
        ...
        // Action row — passes clinic as argument so screens skip the picker
        Row(children: [
          Expanded(child: OutlinedButton.icon(
            onPressed: () => Navigator.pushNamed(
              context, AppRoutes.joinQueue,
              arguments: c,   // pre-selects this clinic
            ),
            style: OutlinedButton.styleFrom(
