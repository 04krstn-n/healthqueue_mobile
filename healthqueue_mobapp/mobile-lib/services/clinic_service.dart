...
class Clinic {
  ...
  factory Clinic.fromJson(Map<String, dynamic> j) {
    ...
    double? lat = _toDouble(
        j['latitude'] ?? j['lat'] ?? location['latitude'] ?? location['lat']);
    double? lng = _toDouble(
        j['longitude'] ?? j['lng'] ?? location['longitude'] ?? location['lng']);
    ...
    if ((lat == null || lat == 0.0 || lng == null || lng == 0.0) &&
      ...
      if (lng == 0.0) lng = _toDouble(coords[0]);
      if (lat == 0.0) lat = _toDouble(coords[1]);
    }
    final address = j['address']?.toString().trim().isNotEmpty == true
        ? j['address'].toString()
    ...
    return Clinic(
      ...
      services: svcs,
      queueLength:
          _toInt(j['queueLength'] ?? j['queueCount'] ?? j['currentQueue'] ?? 0),
      currentWaitingTime: _toInt(j['currentWaitingTime'] ??
          j['estimatedWait'] ??
          j['waitMinutes'] ??
          0),
